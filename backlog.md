...
| **Test-isolate the skills root** | `skillInstall.ts` hardcodes `USER_SKILLS_ROOT` from `os.homedir()`, so Skills Studio tests write to the real `~/.claude/skills` (cleaned in `finally`). Make it overridable via env (e.g. `CAL_SKILLS_ROOT`) so tests use a temp dir and never touch real user state. One-liner; low priority. |
| **Emergent skills: multi-turn capture** | Today `POST /api/skills/propose` distills only the agent's most recent turn. A reusable procedure sometimes spans several turns (explore → act → verify). Let the distiller optionally consume the last N turns of the session. Builds on `src/emergentSkills.ts`. |
...
| 2026-04-25 | GitHub Pages | Pages enabled at `rbjglobal.github.io/clawddesk/` (source: main / root). Repo homepage URL set so the github.com sidebar shows the live URL. README is auto-served as the index by Jekyll. |
...
| 2026-04-29 | Keyboard shortcuts (PR #6) | `c47c849`. ⌘K command palette (filterable list of every modal + action + agent — fuzzy match, ↑↓ navigate, Enter fire, Esc close). Direct shortcuts: ⌘; → Settings, ⌘⇧T → Tasks, ⌘⇧S → Schedules, ⌘⇧M → Memory, ⌘⇧H → History. Esc closes the topmost open modal (peels back layers); skipped when focus is in an editable field. Capture-phase document handler so ⌘K opens the palette even when focus is in a textarea. 3 new Playwright smoke tests (55/55 green). Skip rules: small new client-only surface — Architect + Developer + Reviewer (self), no Perf/Security audits per the cosmetic/no-server-surface skip rule. |
| 2026-04-29 | C05 DONE | Telegram bridge shipped on branch `c05-telegram`. `src/telegram.ts` (~280 LOC, host-agnostic primitive — fetch-based API wrappers + `TelegramListener` class with start/stop, `chunkReply()` paragraph-then-slice helper, `parseAllowedChatIds()`, `TelegramError` with auth/conflict/rate-limit classifiers; designed for Clawless personal-tier lift) + `src/telegramInstance.ts` (singleton bootstrap with `configureTelegram(handler)` / `startTelegram()` / `stopTelegram()` / `restartTelegram()` / `telegramStatus()` / `testTelegramToken()`) + `onTelegramMessage` handler in server.ts (parses `/agent` prefix → resolves agent → CostGuard preflight → fires `query()` with shared `sessionByAgent` state → chunks reply, Markdown→plain fallback on parse error → typing... indicator throughout). Long-poll listener (no public webhook); 25s timeout; exponential backoff to 5min on transient errors; auth-failure / 409-Conflict cleanly stop the loop and surface as ListenerStatus. New routes: `GET /api/telegram/status` + `POST /api/telegram/test`. Settings UI: removed `disabled: true` flag, added Test connection button + live status indicator (mirrors WhisprDesk). `POST /api/settings` detects `telegram.*` key changes and calls `restartTelegram()` — operator can rotate the token without restarting the server. 4 new C05 smoke tests + 1 updated C14-era test (Telegram section is no longer disabled — old test asserting "coming soon" badge replaced with assertion that section is enabled, has Test connection button, fields editable). Reviewer pass: 16 findings, **R9 fixed before merge** (bare `/main` with no body was sending the literal string "/main" as the agent's prompt, burning tokens; rewritten as `prompt = parsed.body` so empty body falls into the "what would you like X to do?" hint path); 5 deferred per reviewer guidance. |
| 2026-04-29 | C05 Perf pass | Audit at `docs/audits/perf-audit-c05.md`: 0 HIGH, 1 MED (fixed inline), 6 LOW. **Inline fix applied** (P1): typing-loop sleep is now wake-able via a `wakeTypingLoop` closure that the finally block calls to `clearTimeout` and resolve immediately. Pre-fix, every Telegram reply paid up to 4s of dead-air latency (avg ~2s) waiting for the residual sleep before sending. Net change ~6 LOC. P2 (`restartTelegram` not serialized) deferred — operator-driven, rare. P3-P7 all measured tiny: `chunkReply` ~78 µs at 50 KB / ~293 µs at pathological 1 MB; `parseAllowedChatIds` ~0.2 µs; idle long-poll CPU ~0 (one fetch per 25s, server-side blocked); `inFlight` Set has correct cleanup with no leak path. Watch list: per-message `configValue` re-read is intentional (token-rotation path); `getMe` on Settings save (~200 ms) is fine for a save-button flow; multi-bot future would need per-key serialization. |
| 2026-04-29 | C05 Security pass | Audit at `docs/audits/security-audit-c05.md`: 0 HIGH, 0 MED, 4 LOW, 6 Info. No inline fixes — every gap was either accept-under-threat-model or a change that should land via Reviewer. Confirmed clean: token never logged (walked every `console.*`); token never returned in API responses (settings GET masks via `maskedSettings()`; `/api/telegram/status` returns no token field; `/api/telegram/test` only returns `botUsername` or generic error string); token never echoed in `TelegramError.description` (Telegram doesn't echo supplied tokens in error strings — verified via 401 response inspection); allowlist enforcement is silent-drop (no `sendMessage` to non-allowed parties); host hardcoded `https://api.telegram.org` (no SSRF surface); zero `db.*` references in C05 code; `parseTelegramCommand` uses only String methods (no eval/Function); 409 Conflict cleanly stops the loop. Watch list for commercial path: S1 (token URL interpolation has no validation — safety load-bearing on undici implementation detail), S2 (`parseAllowedChatIds` over-accepts `1e10`/`0x100`/`007`; theoretical only — Telegram doesn't issue IDs in those ranges), S3 (Telegram-initiated runs don't auto-trigger per-task approval — phone-compromise scenario; document; consider defaulting `Bash\|Write\|Edit\|WebFetch` to require approval for Telegram channel in a future iteration), S4 (`/api/telegram/test` unauthenticated — inherits localhost-trust baseline). |
| 2026-04-30 | Settings UX (PR #8) | `9539f7f`. Three small fixes after dogfooding the Telegram bridge save flow: (1) **per-section "Save section" button** beside Test connection — flashes "✓ Saved" / "✓ Up to date" for 2s after success; (2) **Test connection auto-saves dirty fields first** so the "type token → click Test" path Just Works without remembering to scroll-and-Save first; (3) **dirty-state visual cue** — typed-but-unsaved inputs get an amber border + light tint, cleared on save. Refactored `saveSettings()` to extract `collectSettingsEntriesFrom(scope)` + `postSettingsEntries(entries)` so per-section save reuses the same submit path. Whole-modal "Save changes" button still works as the cross-section escape hatch. Per-section save reloads `settingsState` (meta labels stay accurate) but does NOT re-render the whole modal so focus/cursor isn't lost. 220 LOC across `public/app.js` + `public/style.css`. 59/59 smoke green. Skip rules: cosmetic + small new client-only surface — Reviewer-only. |
| 2026-06-10 | Context pins + MCP UI + Skills panel | Three per-agent capability features shipped on branch `context-mcp-skills`. (1) **Context pins** — `src/contextPins.ts`: per-agent file/snippet content injected into the system prompt every turn via the relocated `augmentedSystemPrompt()` (moved out of memory.ts to compose memory+pins without a circular import). `file` pins re-read from disk each turn (16 KB/file, 32 KB total caps, no-throw on missing/unreadable). Routes GET/POST/DELETE `/api/pins`; 📌 Pins modal. Engine-verified: a pinned snippet instruction was honored verbatim in a live Comms reply. (2) **MCP config UI** — `src/mcpServers.ts`: per-agent stdio/http/sse servers, env/header values masked for the browser but raw at runtime; `mcpOptionsFor()` spreads `{allowedTools (+ mcp__<name> tokens), mcpServers}` into all 5 query sites; `/api/mcp/:id/test` spawns one server and reads connection status from the SDK's `system.init` `mcp_servers` field (snake_case) then aborts before a model turn (10s timeout guard). Live-tested against `@modelcontextprotocol/server-filesystem` → "connected". 🔌 MCP modal with enable toggle + Test. (3) **Skills panel** — `src/skills.ts`: `discoverSkills(cwd)` scans `.claude/skills/*/SKILL.md` (project + user) parsing frontmatter; per-agent enable persists in `agent_skills`; `skillsOptionsFor()` adds `settingSources:['project','user']` + `skills:[names]` when an agent opts in. 🧩 Skills modal. All three wired into the ⌘K palette + Esc modal-stack + active-agent header counts. Reviewer pass (independent agent): 4 findings, 0 HIGH — R2 (mask the `?? server` POST fallback so it can't leak raw secrets) + R3 (10s timeout on MCP test route) fixed before merge; R1 (`settingSources` blast-radius documented in code + this note) ; R4 (id-only route ownership) deferred under personal-use model. 8 new Playwright smoke tests (66/66 green). Watch-list: stale `agent_skills` row would flip `settingSources` on for zero benefit — intersect against `discoverSkills(cwd)` if it bites. **This marks the close of core development per the user — remaining backlog items are nice-to-haves.** |
| 2026-06-11 | Browser automation (Clawless port #1) | First feature of the Clawless cross-pollination. Per-agent toggle giving an agent a real browser via the official `@playwright/mcp` server, behind an allow-list permission gate. `src/browser.ts` (browser_agents table, the URL gate `isUrlAllowed` with private-IP + IPv4-mapped-IPv6 + decimal/hex/octal obfuscation parsing, `browserOptionsFor` injecting the Playwright stdio config with `--isolated`/`--allowed-origins`/`--blocked-origins`/`--block-service-workers`) + server wiring (`agentToolOptions` merges MCP + browser into all 5 query sites; `buildBrowserGuardHook` PreToolUse gate on `browser_navigate`; `mergeHooks`/`hooksOpt` compose with the approval hook; routes `GET/POST /api/browser/:agentId` + `POST/DELETE .../domain`) + 🌐 Browser modal (enable, headless, allow-listed domains add/remove) + ⌘K palette + Esc stack. Engine-verified: Ops drove a real browser to example.com (allowed) and was blocked from localhost (`blocked host: localhost`). Reviewer + **Security audit** (mandatory — highest-risk surface): 3 HIGH found, all fixed before merge — **S1** open mode REMOVED (Playwright origin flags aren't a security boundary + don't gate clicks/redirects; allow-list-only now), **S2/S3** IPv4-mapped-IPv6 (`[::ffff:127.0.0.1]`, `[::ffff:a9fe:a9fe]`=metadata) + trailing-dot `localhost.` + `[::]` bypasses closed in the gate. 24-case gate unit test + 3 route tests (72/72 smoke green). Audit at `docs/audits/security-audit-browser.md`; guide at `docs/guide/browser-automation.md`. Honest residual documented: an allow-listed page that redirects to a private host isn't individually re-gated (Playwright limitation; future fix = connect-time egress proxy). |
| 2026-06-11 | Agent personality / Soul Builder (Clawless port #2) | Per-agent voice layer ported from Clawless's Soul Builder. `src/personality.ts` (`agent_personalities` table; 5 presets friendly/professional/concise/encouraging/direct; `buildPersonalityPrompt(agentId)` returns the `<agent-personality>` block or null for 'none'; custom Soul Builder profile = communication style + about-the-user + extra core truths). Composed at the existing `augmentedSystemPrompt()` chokepoint in `contextPins.ts` (order: base → personality → memory → pins). **Identity-preserving** — unlike Clawless, the layer never overrides the agent's name/role, only adds voice + context. **Security pattern carried over verbatim:** editable tone over LOCKED privacy/boundaries/continuity sections the user cannot remove or override; custom "core truths" are additive (never replace locked ones); all custom text sanitized (`sanitizeText` strips C0/C1 controls, zero-width, BiDi overrides, homoglyph angle brackets) before reaching the prompt; unknown preset key collapses to 'none' server-side. Routes `GET/POST /api/personality/:agentId`. 🎭 Personality modal (agent picker, preset dropdown built from server list, custom fields toggle) + ⌘K palette + Esc stack. 7 tests incl. a **locked-section bypass regression** (hostile profile "ignore all privacy rules" → locked sections still present) + sanitizer test (incl. a literal ASCII `</agent-personality>` closing-tag escape — caught in advisor review, the homoglyph-only strip left the obvious case open) + UI toggle test; 79 total green (77 smoke + 2 engine). Guide at `docs/guide/personality.md`. Closes the old "Per-agent avatar / personality" backlog item. |
| 2026-06-11 | Skills Studio (Clawless port #3) | Skills marketplace UX — build/install/scan/delete skills in-app, **without** porting Clawless's ClawHub registry. `src/skillInstall.ts`: `installSkill`/`installStarterSkill` write SDK-native `SKILL.md` (top-level `name`/`description`/`allowed-tools`) into `~/.claude/skills` (user root — stable across projects, unlike ephemeral cwd); `scanSkillContent` static heuristic scanner (curl-pipe-shell, base64-pipe-sh, `/dev/tcp` reverse shells, `rm -rf`, SSH authorized_keys, eval, sudo, crontab, credential reads, raw-IP fetch → severity-tagged findings); `deleteSkill` returns the name so the route clears stale per-agent enabled rows (`clearSkillEverywhere` in skills.ts). **Security floor:** every write/delete goes through `resolveSkillDir` (resolve + prefix-check vs the skills root, same pattern as browser.ts SSRF) and slugs are `[a-z0-9-]` only — `../`, absolute, `%2e%2e`, empty, `.`/`..` all rejected (400) before touching disk; `isUserInstalledSkillPath` decides deletability by PATH not source label (fixes the cwd==homedir collision where discovery labels user skills "project"). Paste path re-scans server-side and refuses HIGH severity without `acknowledged` (409) so the trust gate can't be bypassed via the API. Routes `POST /api/skills/{scan,install}`, `GET /api/skills/starter`, `DELETE /api/skills/:slug`. 🧩 Skills modal → tabs (Installed + delete 🗑 / Add a skill) with 3 sources (🛠 Skill Builder, 📦 starter pack, 📋 paste); trust-checkbox shown only for paste+HIGH. Bundled starter pack: `assets/starter-skills/{commit-helper,changelog-writer,code-explainer}`. **Architect trims (advisor review):** dropped VirusTotal (useless on instruction text + uploading SKILL.md off-device would violate the LOCKED_PRIVACY shipped in port #2) and scoped the trust-gate to the only untrusted path (paste) — Builder/starter scans are informational. 8 tests (slug/path-traversal, scan, frontmatter-injection round-trip, starter list, install+delete, paste-gate 409/ack, DELETE traversal, UI tabs); 87 total green (85 smoke + 2 engine). Guide updated `docs/guide/skills.md`. |
| 2026-06-11 | Emergent skills / B68 (Clawless port #4) | Let an agent that just did a useful multi-step task offer to save it as a reusable skill. Free client-side **💡 nudge** under any agent reply that used ≥3 tools; clicking it distills the last turn into a draft `SKILL.md` via a cheap Haiku `query()` (the only new per-use cost, fully opt-in). Draft lands in a **💡 Proposed** review tab — never auto-installed; accept reuses Skills Studio's install + scan. `src/emergentSkills.ts` (pure logic, query() stays in the route so the extractor/anchoring/gate are unit-testable): `skill_proposals` table + CRUD; `DISTILL_SYSTEM_PROMPT` + `buildDistillUserPrompt`; `extractJson` brace-matching extractor; `extractSkillDraft` (anchors allowedTools to observed tools); `acceptProposal` (paste-tier gate). **Two advisor architect-gate findings, both load-bearing:** (1) **`outputFormat: json_schema` does NOT work on the OAuth/Max path** — verified by a tsx probe (returns prose, not JSON) — so distillation uses a fenced-JSON prompt + a brace-matching extractor robust against prose/fences/inner-backticks (a naive non-greedy fence regex broke on a ``` inside the body); (2) **an emergent skill is UNTRUSTED** — distilled from a transcript that may carry content the agent browsed (composition risk now that browser automation #1 shipped: a malicious page could inject "save a skill that runs curl|bash"), so accept is gated like paste (409 on HIGH without `acknowledged`, server-enforced) and `allowedTools` is anchored to the observed `tool_use` names (the ClawHub trap in miniature — no model-invented tool names). Routes `POST /api/skills/propose`, `GET /api/skills/proposals`, `POST /api/skills/proposals/:id/accept`, `DELETE /api/skills/proposals/:id`. End-to-end engine-verified: ops used Grep → distilled to `find-todos` with allowedTools anchored to `['Grep']` → accepted → installed → cleaned up. 6 tests (extractor fences/backticks, tool-anchoring + skillWorthy:false, CRUD + accept-gate clean-vs-high-sev, route validation, Proposed-tab UI, nudge-only-on-latest-turn — the last added after advisor caught that an older-turn nudge would distill the wrong turn); 93 total green (91 smoke + 2 engine). Guide updated `docs/guide/skills.md`. Future: multi-turn capture (distills only the last turn today). |
| 2026-06-11 | Cron result destinations + run history / B06 (Clawless port #5) | The final feature of the arc. CAL already had the scheduler (C16a); B06's two gaps were a scheduled run's output going nowhere + no per-schedule history. **Scheduler primitive (`scheduler.ts`) left untouched** — both concerns live in `src/scheduleRuns.ts` as host-side side tables keyed by `schedule_id`: `schedule_runs` (status + full output + delivery + cost, kept-last-50 retention) and `schedule_destinations` (in-app / file / telegram). `fireScheduledTask` records each run + delivers best-effort. **Destinations:** in-app (history only), file (append with timestamp header), telegram (reuses C05 `sendMessage` to an explicit chatId). **Run vs delivery outcome are separate** — a successful run whose delivery fails stays success, with the delivery error captured on the row (never thrown out of the fire). **Advisor architect-gate blocker:** the file destination is a write-side egress of attacker-influenceable content (unattended run + browser-fetched output post-#1), so — unlike the pins *read* analogy — it's **confined to `~/.clawddesk/reports/`** via `resolveReportPath` (resolve + prefix-check, same floor as browser/skills); `../`/absolute/escape rejected, so the "append to `~/.zshrc`"/`.mcp.json`/`SKILL.md` execute-on-read class is impossible. **Orphan cleanup** (the #3 stale-row lesson): `DELETE /api/schedules/:id` calls `clearScheduleData` to drop the run/destination rows the primitive doesn't know about. Routes `GET /api/schedules/:id/runs`, `GET/POST /api/schedules/:id/destination`; destination also accepted at create (invalid one rolls the create back). 🕒 Scheduler modal: destination picker in the form (file/telegram inputs toggle) + per-schedule History expander showing run transcripts + a destination chip. End-to-end engine-verified: run-now → run recorded (success, delivery ok) → output appended under reports dir with timestamp → schedule+history cleaned up. 6 tests (path confinement incl. literal-`~` is confined-not-escaped, dest validation, recordRun/deliver-to-file/clear, create-rollback + runs-404, delete-clears-history, UI picker toggle); 100 total green (98 smoke + 2 engine). Guide updated `docs/guide/scheduler.md`. **Closes the 5-feature Clawless cross-pollination arc** (#1 browser, #2 personality, #3 skills studio, #4 emergent skills, #5 cron destinations). |
| 2026-06-16 | Stabilization session | Aggressive regression + cleanup pass. Fixed a boot blocker (Node 25 better-sqlite3 ABI rebuild). Multi-agent review (correctness/dead-code/security/consistency + adversarial verification): 19 raw → 17 confirmed. Merged PRs #25 (backlog split into done/open), #26 (65 lines dead code removed), #27 (correctness + costGuard typing → tsc 0 errors), #28 (scan-gate RCE-bypass hardening), #29 (wrap-up). Now: tsc clean (0 errors), 0 unused locals/params, 100/100 tests green. 1 finding deferred (skill overwrite-on-collision UX). Report: docs/audits/stabilization-2026-06-16.md. |
