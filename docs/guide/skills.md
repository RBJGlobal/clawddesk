...
ClawdDesk surfaces the [Agent Skills](https://code.claude.com/docs/en/skills) it can find on your machine and lets you turn them on per agent.
...
The SDK loads skills only when `settingSources` includes `'project'` or `'user'`, and the `skills` option then filters **which** discovered skills actually load into the system prompt. ClawdDesk mirrors that exactly ([`src/skills.ts`](../../src/skills.ts)):
